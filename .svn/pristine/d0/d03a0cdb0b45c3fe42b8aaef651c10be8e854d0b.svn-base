			...
			$.ajax({
				...
				success:function(list){
					...
					$.each(list,function(i,v){
						...
						code+= "<td class='align-middle text-center'><img src='/resources/image/trash3.svg'></td></tr>";
...
<form:form modelAttribute="AtrzLineInfoVO" id="frm"
	action="/edoc/edocUpdatePost?${_csrf.parameterName}=${_csrf.token}"
	...
	<table class="m-2"
		...
		<tr>
			...
			<td><button type="button" class="btn btn-outline-dark"
					style="font-size:1.2em; width: 130px;">임시저장</button></td>
			<td><button type="button" class="btn btn-outline-dark"
		...
		<table class="px-5 mx-5 mx-auto"
			...
			<tbody>
				<tr>
					...
					<td
						style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 16px; font-weight: normal; vertical-align: middle;"
						colspan="3"><input type="text" size="45" name="edocVO.edocTtl" value="${list.edocVO.edocTtl}"></td>
				</tr>
					...
					<td
						...
						colspan="4"><span style="width: 100%; min-width: 200px;">
							<textarea rows="100" cols="100" id="DRFT_CN" name="edocVO.dfrtVO.drftCn"
								style="width: 100%; height: 550px;">${list.edocVO.dfrtVO.drftCn}</textarea>
					</span></td>
				</tr>
			</tbody>
	...
	<input type="hidden" name="edocVO.edocFormClsf" value="${list.edocVO.edocFormClsf}" />
	<input type="hidden" name="edocVO.edocNo" value="${list.edocVO.edocNo}" />
	<input type="hidden" name="atrzLineInfoNo" value="${list.atrzLineInfoNo}" />
	...
	$(function() {
		...
		$.ajax({url : "/org/orgAdminEmp",
					...
					success : function(result) {
						...
						$.each(result,function(i, v) {
							str += "<li><img src='/resources/image/file.gif'><a class='file file2' data-empno='"+v.empNo+"' >"
									+ v.empNm
									+ " "
									+ v.jbpsNm
									+ "</a></li>"
